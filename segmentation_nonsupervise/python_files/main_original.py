...
im = cv2.resize(image, new_dim, interpolation= cv2.INTER_LINEAR)
print(f'im:{im.shape}')
...
path_image_name = f"./dataset_patch/output_individual_new/min_labels3_{args.minLabels}_lr_{args.lr}_cont_{args.stepsize_con}_nchannels_{args.nChannel}_maxIter_{args.maxIter}_{image_name}"
print(f'save image:{path_image_name}')
print(f"im_target:{im_target.shape}")
print(np.unique(im_target))
print(f"im_target_rgb shape:{im_target_rgb.shape}")

if im_target_rgb is not None:
    cv2.imwrite(path_image_name, im_target_rgb )
    #Image.save(path_image_name )
