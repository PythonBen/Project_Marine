...
# command for batch prcessing image or individual picture
# for batch processing image
#python main_ref.py --nChannel 60 --maxIter 10 --maxUpdate 10
# --minLabels 5 --batch_size=4 --lr 0.1 --nConv 2 --visualize 0
# --input ./dataset --stepsize_sim 1 --stepsize_con 1

# for individual images
#python main_original.py --nChannel 40 --maxIter 20 --minLabels 4
# --lr 0.1 --nConv 2 --visualize 1 --input ./dataset/test_resized/resize_000261.jpg
...
img_list = sorted(glob.glob(args.input+'/train/*'))[0:8]
...
label_colours = np.random.randint(255,size=(100,3))
#test_img_list = sorted(glob.glob(args.input+'/test_resized/*'))
test_img_list = sorted(glob.glob(args.input+'/valid/*'))
#if not os.path.exists(os.path.join(args.input, 'result/')):
#    os.mkdir(os.path.join(args.input, 'result/'))

if not os.path.exists(os.path.join(args.input, f'size_{size[0]}_result_bs_{args.batch_size}_maxIter_'
                                               f'{args.maxIter}_minLabels_{args.minLabels}_con_'
                                               f'{args.stepsize_con}_nchannel_{args.nChannel}_maxupdate_{args.maxUpdate}/')):
    os.mkdir(os.path.join(args.input, f'size_{size[0]}_result_bs_{args.batch_size}_maxIter_'
                                      f'{args.maxIter}_minLabels_{args.minLabels}_con_'
                                      f'{args.stepsize_con}_nchannel_{args.nChannel}_maxupdate_{args.maxUpdate}/'))
...
for img_file in tqdm.tqdm(test_img_list):
    ...
    cv2.imwrite(os.path.join(args.input,
                             f'size_{size[0]}_result_bs_{args.batch_size}_maxIter_' 
                             f'{args.maxIter}_minLabels_{args.minLabels}_con_' 
                             f'{args.stepsize_con}_nchannel_{args.nChannel}_maxupdate_{args.maxUpdate}/') + os.path.basename(img_file), inds_rgb)
