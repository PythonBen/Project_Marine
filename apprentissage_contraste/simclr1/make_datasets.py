...
#pathdat = '/home/lepersbe/Narval/images_data/total_images/'
P = Parameters()
BS_US = P.BS_US
...
RATIO = P.RATIO
# ****************************************************
# make a dataset with labelles for Supervised learning
...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        #image = read_image(self.get_x(idx)) #uint8
        image = Image.open(self.get_x(idx)).convert('RGB') #PIL image
        #image = image.double() #float64
        #print(image.dtype)
...
#unlabeled_data_train = unlabeled_data[0:b1]
#unlabeled_data_val = unlabeled_data[b1:]
# the constrative model will be trained on the unlabeled data
