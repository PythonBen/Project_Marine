...
# -*- coding: utf-8 -*-
"""
Created on Tue May 24 14:59:23 2022
...
CURRY = False
if CURRY:
    path_root = Path('/home/lepersbe/Narval/submeeting_2022/')
else:
...
dataframes = ['df_boul2_enc.pkl', 'df_p2_enc.pkl', 'df_p3_enc.pkl', 'df_p4_enc.pkl', 'df_total_su_enc.pkl']
...
df = pd.read_pickle(path_dataframe/dataframes[N])
if TOY:
   df = df.iloc[0:100]
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, dataframe, path_data, transform=None, target_transform=None, Size=0):
    ...
    def get_x(self,i):
        ...
        im = (self.dataframe.iloc[i]['fname'])                       # /dataset_su/boulouris2/00000
        imparent = im.parent.as_posix()                              # /dataset_su/boulouris2
        path_parts = im.parent.parts                                 # ('/', '/dataset_su/, '/boulouris2/)
        fold_name = path_parts[-1]

        if fold_name == 'boulouris2':
            frames = '0_3525'
        elif fold_name == 'pyramide3':
            frames = '0_380'
        else:
            frames = '0_900'

        folder_im = fold_name + '_frames_' + frames + self.im_size[self.Size]
        imfull = self.path_data.as_posix()  + imparent + '/' + folder_im + im.name +'.jpg'
...
#print(f'len train:{len(train_ds)}, len val:{len(val_ds)}')
...
#print(f'len list_images:{len(list_images_total)}')
#print(list_images_total[0:5])
# *************************
# make a unlabeled dataset for unsupersvised learning
# full dataset:
# train: 15035 images, val: 3759 images
# toy dataset 1% or 10%
#counting_in_folder()
#RATIO = 0.8
...
#print(f'len train:{len(list_train)}, len val:{len(list_val)}')
...
if __name__ == "__main__":
    ...
    # to do make a toy unsupervised dataset, 0.01% for example
